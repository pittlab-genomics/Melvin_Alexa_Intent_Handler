...
async function build_ind_gain_compare_tcga_response(handlerInput, melvin_state, state_diff) {
    const image_list = [];
    const response = await get_ind_gain_compare_tcga_stats(handlerInput, melvin_state);
...
async function build_ind_gain_compare_response(handlerInput, params, state_diff) {
    ...
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_ind_gain_compare_tcga_response(handlerInput, params, state_diff);
