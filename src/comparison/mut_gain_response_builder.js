...
async function build_mut_gain_compare_tcga_response(handlerInput, melvin_state, compare_state, state_diff, opts) {
    const image_list = [];
    const response = await get_mut_gain_compare_tcga_stats(handlerInput, melvin_state, compare_state);
    ...
    return build_ssml_response_from_nunjucks("dtype_compare/mut_gain.njk", nunjucks_context, opts);
}

async function build_mut_gain_compare_response(handlerInput, params, compare_state, state_diff, opts={}) {
    ...
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_mut_gain_compare_tcga_response(handlerInput, params, compare_state, state_diff, opts);
