...
async function build_snv_cna_compare_tcga_response(handlerInput, melvin_state, compare_state, state_diff) {
    const image_list = [];
...
async function build_snv_cna_compare_response(handlerInput, params, compare_state, state_diff) {
    ...
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_snv_cna_compare_tcga_response(handlerInput, params, compare_state, state_diff);
