...
async function build_snv_cna_compare_tcga_response(handlerInput, melvin_state, state_diff) {
    const image_list = [];
    const response = await get_snv_cna_compare_tcga_stats(handlerInput, melvin_state);
    const nunjucks_context = {
        melvin_state: melvin_state,
        state_diff:   state_diff,
        response:     response
    };
    const speech_ssml = build_ssml_response_from_nunjucks("dtype_compare/snv_cna.njk", nunjucks_context);
...
async function build_snv_cna_compare_response(handlerInput, params, state_diff) {
    ...
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_snv_cna_compare_tcga_response(handlerInput, params, state_diff);
