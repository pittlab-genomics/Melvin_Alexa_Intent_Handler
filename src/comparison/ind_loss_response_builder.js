...
async function build_ind_loss_compare_tcga_response(handlerInput, melvin_state, compare_state, state_diff, opts) {
    const image_list = [];
    ...
    return build_ssml_response_from_nunjucks("dtype_compare/ind_loss.njk", nunjucks_context, opts);
}

async function build_ind_loss_compare_response(handlerInput, params, compare_state, state_diff, opts={}) {
    ...
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_ind_loss_compare_tcga_response(handlerInput, params, compare_state, state_diff, opts);
