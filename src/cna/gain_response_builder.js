...
async function build_gain_tcga_response(handlerInput, melvin_state) {
    ...
    const speech_ssml = build_ssml_response_from_nunjucks("cna/gain_tcga.njk", nunjucks_context);
...
async function build_gain_compare_tcga_response(handlerInput, melvin_state, compare_params, sate_diff) {
    const image_list = [];
    ...
    const speech_ssml = build_ssml_response_from_nunjucks("cna/gain_compare_tcga.njk", nunjucks_context);
    add_gain_tcga_plot(image_list, melvin_state);
...
async function build_gain_compare_response(handlerInput, params, compare_params, sate_diff) {
    console.info(`[build_gain_compare_response] params: ${JSON.stringify(params)}`);
    let response = {};
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_gain_compare_tcga_response(handlerInput, params, compare_params, sate_diff);
