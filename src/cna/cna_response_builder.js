...
async function build_cna_response(handlerInput, params) {
    console.info(`[build_cna_response] params: ${JSON.stringify(params)}`);
    let response = {};
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        ...
        throw melvin_error(
            `[build_cna_response] not implemented: ${JSON.stringify(params)}`,
            MelvinIntentErrors.NOT_IMPLEMENTED,
            DEFAULT_NOT_IMPLEMENTED_RESPONSE
        );
    } else {
        throw melvin_error(
            `[build_cna_response] invalid state: ${JSON.stringify(params)}`,
...
async function build_cna_compare_response(handlerInput, params, compare_params, state_diff) {
    console.info(`[build_cna_compare_response] params: ${JSON.stringify(params)}`);
    let response = {};
    if (params[MelvinAttributes.DSOURCE] === DataSources.TCGA) {
        response = await build_cna_compare_tcga_response(handlerInput, params, compare_params, state_diff);
...
const add_cna_tcga_plot = function (image_list, params) {
    const cna_url = new URL(`${MELVIN_EXPLORER_ENDPOINT}/analysis/cna/tcga/cna_plot`);
